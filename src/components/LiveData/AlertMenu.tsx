...
export const useAlerts = () => {
  ...
  const checkForAlerts = (data: LiveDataItem[]) => {
    ...
    const highCurrentItems = data.filter(item => item.total_current >= 15.0 && item.state !== 'off');
  ...
  const processAlert = (newData: LiveDataItem) => {
    ...
    // Don't create an alert if the machine state is "off"
    if (newData.state === 'off') return;
  ...
  const addStateChangeAlert = (machineId: string, previousState: string, newState: string, timestamp: string) => {
    ...
    // Only add the alert if the machine is not off
